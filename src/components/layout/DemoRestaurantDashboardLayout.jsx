...
export function DemoRestaurantDashboardLayout() {
    const navigate = useNavigate();
    const location = useLocation();

    // Use Restaurante Delicias (third company) for restaurant demo
    ...
    return (
        <div className="flex h-screen w-full bg-slate-50 overscroll-none supports-[height:100dvh]:h-[100dvh]">
                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                    {menuItems.map((item) => (
                        <NavLink
                            key={item.path}
                            to={item.path}
                            end={item.path === '/demo/restaurante/dashboard'}
                            className={({ isActive }) =>
                                cn(
            ...
            <div className="flex flex-1 flex-col overflow-hidden relative">
                ...
                <nav className="flex-none h-16 border-t border-slate-200 bg-white md:hidden grid grid-cols-4 z-50">
                    {menuItems.map((item) => (
                        <NavLink
                            key={item.path}
                            to={item.path}
