    ...
    const loadDashboardData = async () => {
        ...
        try {
            const [favs, qts, revs] = await Promise.all([
                supabase.from('favorites').select('*, target:companies(*)').eq('user_id', user.id).limit(4),
                supabase.from('quotes').select('*, company:companies(*)').eq('customer_id', user.id).order('created_at', { ascending: false }).limit(5),
            ...
            const { count: totalQuotes } = await supabase.from('quotes').select('id', { count: 'exact' }).eq('customer_id', user.id);
        ...
        <div className="space-y-8 pb-12">
            ...
            <div className="bg-white p-8 sm:p-10 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden flex flex-col sm:flex-row items-center gap-8">
                ...
                <div className="relative z-10 text-center sm:text-left flex-1 min-w-0">
                    ...
                    <h1 className="text-4xl font-black text-slate-900 mb-2 tracking-tight">
                        ¡Hola, <span className="text-primary-600">{profile?.full_name?.split(' ')[0] || 'Cliente'}</span>! 👋
                    </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <Link to={`/catalogo/${fav.target?.slug}`}>
                                                    <h4 className="font-bold text-slate-900 truncate uppercase tracking-tight mb-1">{fav.target?.name}</h4>
                                                </Link>
                ...
                <div className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-xl font-black text-slate-900 flex items-center gap-3">
                            <ShoppingBag size={24} className="text-blue-500" /> Últimos Pedidos
                        </h2>
                        <Link to="/dashboard/cliente/pedidos" className="text-[10px] font-black text-primary-600 hover:underline uppercase tracking-widest">Ver Historial</Link>
                    </div>
                    ...
                    <div className="space-y-3">
                        {recentQuotes.length > 0 ? (
                            ...
                            <div className="text-center py-12 bg-white rounded-[2.5rem] border border-slate-100 border-dashed">
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sin pedidos activos</p>
                            </div>
                        )}
